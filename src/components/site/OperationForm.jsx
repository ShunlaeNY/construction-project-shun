...
export default function OperationForm({
      ...
      const fetchSiteInfo = async () => {
        try {
          const response = await axios({
            method: "GET",
            url: `http://localhost:8383/site/${siteId}`,
            header: {
          ...
          console.error("Error getching site information:", error);
        }
      };
      fetchSiteInfo();
    }
  ...
  const validateDateRange = () => {
    ...
    const siteStartDate = new Date(siteInfo.startDate);
    const siteEndDate = new Date(siteInfo.endDate);
    const operationStartDate = new Date(formData.startDate);
    const operationEndDate = new Date(formData.endDate);

    const isStartDateValid =
      operationStartDate >= siteStartDate && operationStartDate <= siteEndDate;
    const isEndDateValid =
      operationEndDate >= siteStartDate && operationEndDate <= siteEndDate;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateDateRange()) {
      setDateError("Operation dates must be within the site's date range");
      return;
    }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="editScheduleTab  ">
        ...
        <div className=" editScheduleTabContent">
          <div>
            <label
              htmlFor="operationtypesId"
              className="inputLabel inputLabel2"
            >
